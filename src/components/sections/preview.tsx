...
import { Reveal, RevealCard, cardRevealDelay } from "../ui/reveal";
...
export default function Preview() {
  ...
  return (
    ...
    >
          ...
          <div className="flex lg:grid lg:grid-cols-3 gap-4 md:gap-5 lg:gap-6 pr-4 md:pr-10 lg:pr-0 lg:px-0 mx-auto min-w-max lg:min-w-full">
            {previewItems.map((item, index) => (
              <RevealCard
                key={index}
                index={index}
                className={`group relative aspect-40/28 overflow-hidden bg-[#F9FAFB] border-[1.5px] border-[#E5E7EB] rounded-lg max-w-[400px] w-[75vw] md:w-[40vw] lg:w-full shadow-card transition-all duration-300 hover:border-[#E1E3E5] hover:bg-white hover:shadow-xl hover:shadow-[#0047FF]/5 ${item.isLocked ? "" : "p-6"}`}
              >
        ...
        <Reveal delay={cardRevealDelay(previewItems.length)} className="flex flex-col items-center gap-3">
          <Button
